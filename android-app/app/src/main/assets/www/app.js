...
const el = {
  ...
  paymentModel: document.getElementById("paymentModel"),
...
function renderTable() {
  ...
  el.table.innerHTML = list
    .map(
      (r) => `
      <tr>
        <td data-label="Name"><strong>${escapeHtml(r.name)}</strong><br /><small>${escapeHtml(r.paymentDetails || "-")}</small></td>
        <td data-label="Role">${escapeHtml(r.role)}</td>
        <td data-label="Level">${escapeHtml(r.resourceLevel || "-")}</td>
        <td data-label="Type"><span class="badge">${escapeHtml(r.resourceType)}</span></td>
        <td data-label="Status"><span class="badge status-${sanitizeStatusClass(r.status)}">${escapeHtml(r.status)}</span></td>
        <td data-label="Referred By">${escapeHtml(r.referredBy || "-")}</td>
        <td data-label="Allocation">${r.allocation}%</td>
        <td data-label="Commercial">
          ${formatMoney(Number(r.contractedPayment))}
          <br /><small>${escapeHtml(r.paymentModel || "Monthly Retainer")}</small>
          <br /><small>${escapeHtml(r.paymentCycle)}</small>
        </td>
        <td data-label="Contract / Offer">
        ...
        <td data-label="Payment">${escapeHtml(r.paymentStatus)}<br /><small>${escapeHtml(r.paymentMethod)}</small></td>
        <td data-label="Dates">
...
function resetForm() {
  ...
  el.paymentModel.value = "Monthly Retainer";
...
async function getFormPayload(existingContractDocument) {
  ...
  return {
    ...
    paymentModel: el.paymentModel.value,
...
function populateForm(item) {
  ...
  el.paymentModel.value = item.paymentModel || "Monthly Retainer";
